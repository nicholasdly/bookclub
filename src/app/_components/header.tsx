...
export default async function Header() {
  ...
  return (
    <header className="border-b">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-2 md:py-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 md:gap-16">
          <div className="block md:hidden">
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon">
                  <MobileMenuIcon className="w-7 h-7"/>
                </Button>
              </SheetTrigger>
              <SheetContent side="left">
                ...
                {links.map((link, index) => ((link.isPublic || user) && (
          ...
          <div className="hidden md:flex md:gap-4">
            {links.map((link, index) => ((link.isPublic || user) && (
              <Button key={index} variant="ghost" className="text-lg" asChild>
                <Link href={link.href}>{link.text}</Link>
              </Button>
            )))}
          </div>
